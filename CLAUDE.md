...
These configs run on the user's live desktop; an agent can't test them. The user
is the test harness — so hand them something testable.

- **Editing the file is not applying it.** Symlinks make the change live, but
  the process still has to re-read it: `hyprctl reload` for `hypr/`,
  `systemctl --user restart kanata`, `systemctl --user daemon-reload` after
  touching `systemd/user/*`. Say which one is needed.
- **State the observable check**, not just the command — what to press and what
  should happen ("press SUPER+N — a floating window should appear centered").
  "It should work now" is not a check.
- **Don't commit until the user confirms it works.** They say so explicitly ("it
  works, commit"). Until then the change is unverified, however obvious it
  looks.
