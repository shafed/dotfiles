...
-- Создает grabber для цепочки клавиш (может быть вложенным)
local function create_grabber(chain_actions, timeout_seconds)
    local grabber
    grabber = keygrabber.run(function(mod, key, event)
        if event == "release" then return end
        
        if chain_actions[key] then
    ...
    gears.timer.start_new(timeout_seconds or 2, function()
        keygrabber.stop(grabber)
        return false
    end)
end

-- Простая цепочка (одноуровневая)
local function create_key_chain(timeout_action, chain_actions, timeout_delay)
    return function()
        local action_executed = false
        
        local timer = gears.timer {
            timeout = timeout_delay or 0.3,
            autostart = true,
            single_shot = true,
            callback = function()
                if not action_executed and timeout_action then
                    timeout_action()
                end
            end
        }
...
-- Создает под-цепочку (sub-chain)
local function create_sub_chain(action, sub_actions, delay)
    return function()
        local action_executed = false
        
        -- Таймер для действия по умолчанию
        local timer = gears.timer {
            timeout = delay or 0.5,
            autostart = true,
            single_shot = true,
            callback = function()
                if not action_executed and action then
                    action()
                end
            end
        }
        
        -- Запускаем под-цепочку
        gears.timer.start_new(0.05, function()
            local grabber
            grabber = keygrabber.run(function(mod, key, event)
                if event == "release" then return end
                
                if sub_actions[key] then
                    action_executed = true
                    timer:stop()
                    keygrabber.stop(grabber)
                    sub_actions[key]()
...
-- Функция для переключения между окнами одного класса
local function switch_within_class()
    local c = client.focus
    if not c then return end
    
    local clients_in_class = {}
    for _, cl in ipairs(client.get()) do
        if cl.class == c.class and cl:isvisible() then
            table.insert(clients_in_class, cl)
        end
    end
    
    if #clients_in_class > 1 then
        for i, cl in ipairs(clients_in_class) do
            if cl == c then
                local next_client = clients_in_class[(i % #clients_in_class) + 1]
...
{description = "снимок области в буфер (Win+Shift+S)", group = "screenshot"}),
...
awful.rules.rules = {
    ...
    { rule_any = {
        instance = {
          "DTA",  -- zen-browser addon DownThemAll.
    ...
    -- Set zen-browser to always map on the tag named "2" on screen 1.
