...
`networking`, `storage`, `printing`, `study` and its private `hardware-shafed`
driver profile. The `arch-laptop` machine reuses the workload profiles while
replacing `desktop` with `laptop`; it intentionally omits `printing` and the
desktop hardware profile. Add `printing` to that machine's profile list before
