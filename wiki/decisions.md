...
- **Decision**: use native kitty for managed sessions and keep no tmux config in
  this repository. Tmux may still be used occasionally with its built-in
  defaults. Navigation hotkeys are sent by kanata as `C-S-*`.
...
- **Rejected**: keeping tmux as a configured session layer (extra layer,
  theme/navigation desync); removing the tmux program entirely (its defaults
  remain useful occasionally); wezterm's multiplexer (wezterm is also out of
  use).
- Trade-off: managed sessions are tied to kitty, while occasional tmux usage
  deliberately gets no dotfiles customization. See [sessions](sessions.md),
  [kitty](kitty.md).
...
- The remaining `awesome/` legacy was removed on 2026-09-02 after Hyprland had
  fully replaced it.
