...
- **Plugins clone themselves** if `$ZSH` or a plugin is missing, so a fresh
  machine needs no plugin step in `bootstrap.sh` — the shell bootstraps itself.
...
- ⚠️ **`ZVM_LAZY_KEYBINDINGS=false` must be assigned *before* sourcing
  oh-my-zsh, never inside `zvm_config()`.** By default the plugin defers all
  `vicmd` bindings and flushes them only when the shell *first enters normal
  mode* — so opening a pane, running `claude`, and exiting means `^[t` was never
  ...
  end, so you get the lazy path disabled *and* no binding installed — strictly
...
- ⚠️ **`ZVM_LINE_INIT_MODE=$ZVM_MODE_INSERT`** — otherwise each prompt inherits
  ...
  NORMAL. This one *does* belong in `zvm_config()`, because `$ZVM_MODE_INSERT`
