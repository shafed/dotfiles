...
(symlink `~/.zshrc → ~/dotfiles/zsh/zshrc`, see [[bootstrap]]).

## Why it's built this way

- **oh-my-zsh, `gruvbox-material` theme** — a single gruvbox across all tools
  (kitty/nvim/fzf), see [[theming]]. Colors for `zsh-syntax-highlighting`,
...
- **Plugins auto-install** (`zsh-autosuggestions`,
  ...
  ([[bootstrap]]), so the bootstrap is baked into the config itself.
...
- **Alt-t (`kitty_other_window`)** mirrors nvim's `<M-t>`: jump to the neighboring kitty
  ...
  `vim.g.tmux_pane_direction` in nvim. See [[kitty]], [[sessions]].
...
- nvim: `vi`=nvim, `vif`/`vip` — open config/plugins; `neobean`/`nb` —
  alternate `NVIM_APPNAME` (see [[nvim]]).
...
is now its own symlink `~/.config/nvim → ~/dotfiles/nvim` ([[bootstrap]]).
...
handled by kanata ([[keymap]]); the zsh mechanism was redundant.
